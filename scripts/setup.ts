...
const main = async () => {
    ...
    let platform;
    const os: any = await inquirer.prompt([
        {
            type: 'list',
            name: 'os',
            message: 'Select os',
        ...
        },
        {
            ...
            choices: (ans: any) => _.find(choices, { value: ans.os }).versions
        }
    ]);
    const browser: any = await inquirer.prompt([
        {
            type: 'list',
            name: 'browser',
    ...
    const suite: any = await inquirer.prompt([
        {
            type: 'list',
            name: 'path',
            message: 'Select test suite to run',
            choices: readdirSync(__dirname + '/../test')
                .filter(f => statSync(join(__dirname, '..', 'test', f)).isDirectory())
                .map(e => ({
                    name: e.replace(/_/g, ' '),
                    value: join(__dirname, '..', 'test', e)
                }))
        }
    ]);

    const featureSuite = basename(suite.path) === 'features';
    ...
    const grepedTags = execSync(`grep -ho -e "${grepRegExp}" ${join(suite.path, '**/*.feature')}`);
    const tagChoices = _.uniq(grepedTags.toString().split('\n'))
        .filter(e => e.length)
        .sort()
        .map(e => ({ name: e, value: e }));

    const tags: any = await inquirer.prompt([
        {
            type: 'list',
            name: 'selected',
    ...
    const suiteConfig = { ...capabilitiesDict, ...os, ...browser.browser };
